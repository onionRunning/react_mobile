...
export class NormalText extends Component<Props> {
  ...
  render() {
    ...
    const val: number | string | JSX.Element = userType ? this.handTypeValue(userType, value) : value
    return (
      <div className="normal-text-com">
        <span>{`${title ? title + ' :' : ''}`}</span>
        <Tooltip title={val} trigger="contextMenu">
          <p className={widthStyle!}>{val && isString(val) ? this.showValue(val, userType) : val === 0 ? 0 : val}</p>
        </Tooltip>
      </div>
    )
  }
