...
export class UserInfo extends Component<Props> {
  componentDidMount() {
    // 接口逻辑提到上层组件
    console.log(this.props.userDetail.user_msg)
  }

  render() {
    const { score_card_results = [], device_info = {}, order_msg = {}, need_verify_bank = false } =
      this.props.userDetail || {}
    const { personal = {}, work = {}, id = {}, contact = {}, account = {}, credit_info = {} } =
      this.props.userDetail || {}
    return (
      <div className="information-content">
        ...
        <div className="right">
          ...
          <AccountInfo {...this.props} data={{ ...account, ...order_msg }} isShowBtn={need_verify_bank} />
  ...
  getSupplementInfo = () => {
    // const { sup_cert } = this.props.userDetail.user_info
    // // 旧数据不返回该字段，为null
    // if (!sup_cert) {
    //   return
    // }
    // const { salary_img_1, bank_stmt_img_1, bank_stmt_img_2, bank_stmt_img_3 } = sup_cert
    // // 新数据会返回该字段，但是图片字段为空字符串
    // if (!salary_img_1 && !bank_stmt_img_1 && !bank_stmt_img_2 && !bank_stmt_img_3) {
    //   return
    // }
    // return <SupplementInfo {...this.props} showPicture={this.showPicture} data={sup_cert || {}} />
  }
  ...
  //   // 关闭图片展示
  hidePicture = () => {
    // this.props.dispatch(createHideImgView())
  }
}
